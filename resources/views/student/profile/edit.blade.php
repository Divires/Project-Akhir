...
<div>
    ...
    <div class="overflow-x-auto border border-[#e2e2e2] rounded-lg p-6 bg-white text-sm shadow w-full">
        <form action="{{ route('student.profile.update') }}" method="POST" enctype="multipart/form-data" class="grid md:grid-cols-3 gap-6">
            @csrf
            @method('PUT')

            <!-- Foto Profil di kiri -->
            <div class="flex flex-col items-center gap-3 col-span-1">
                <img src="{{ $student->photo ? asset('storage/' . $student->photo) : '/default-profile.png' }}"
                     alt="Foto Profil"
            ...
            <div class="md:col-span-3 pt-4 flex gap-4">
                <a href="{{ route('student.profile.index') }}" class="px-6 py-2 rounded border border-[#e2e2e2] text-[#2c2c2c] hover:bg-[#f1f1f1] transition">
                    Batal
                </a>
